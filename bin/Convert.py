...
class Convert(object):
    def __init__(self,flysight,fver,slate,comp,output,mp4,roundN,res,seq):
        ...
        self.src_dir = roundN
        self.fout_dir = roundN+"_scored"
        self.fout = output
        self.mp4 = mp4
        self.seq = seq
        if flysight:
            self.plot_run, self.comp_run, self.comp_time = pf.get_flysight(pf.convert_flysight(roundN+"/"+flysight,fver))
        else:
        ...
        # comp_end = comp
        self.comp_end = comp + self.comp_time + td(seconds=2*self.DEFAULT_COMP_PAD)
    def convert_format(self):
        os.system("rm -rf %s/__%s_tmp__.mp4"%(self.fout_dir,self.fout))
        os.system("rm -rf %s/slate_%s.mp4"%(self.fout_dir,self.fout))
        scale_crop_opt = "-vf \"crop=%s,scale=%s\""%(self.orig_RES,self.RESOLUTION)
        # code.interact(local=locals()) 
    ...
    def save_video(self,scores,rtype,dock_list,freelist):
        threaded_camera = tv.ThreadedVideo('%s/__%s_tmp__.mp4'%(self.fout_dir,self.fout), comp_start=td(seconds=self.comp_start), comp_time=self.comp_time, fout=self.fout_dir+"/"+self.fout, scores=scores, dock_list=dock_list, freelist=freelist,mode=rtype,seq=self.seq)
        while not threaded_camera.done:
            try:
                threaded_camera.show_frame()
            except AttributeError:
